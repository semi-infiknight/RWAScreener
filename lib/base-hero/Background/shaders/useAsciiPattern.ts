...
import { DoubleFBO } from '../../hooks/useDoubleFbo';
import { useShader } from '../../hooks/useShader';
import { useUniforms } from '../../hooks/useUniforms';
import { saveGlState } from '../../hooks/useSaveGlState';
...
export function useAsciiPattern({
  ...
  useEffect(() => {
    ...
    textureLoader.load('/base-hero/pat3.png', (texture) => {
    ...
    const altPatternUrl = altPattern?.url ?? '/base-hero/pat7-colored.png';
  ...
  const render = useCallback<RenderCallback>(
    (state) => {
      ...
      uniforms.uImageDimensions.value = imageTexture?.image
        ? new THREE.Vector2(
            (imageTexture.image as { width: number; height: number }).width,
            (imageTexture.image as { width: number; height: number }).height,
          )
