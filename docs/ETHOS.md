...
10. **Vesper interest** — v0 live: reads her @/replies/quotes (`GET /api/vesper-interest`). Harvest same scan is **quote/reply only** (not mention-only `@`). Homepage membership is the **post** (DBC / pad / named drama), never a people denylist or people allowlist. A `@semiii` DBC/quote-screener tweet stays; their other posts do not. Tracked pads/builders still bypass BGE for announcements. No Following/Likes API (app bearer). Example: https://x.com/vesper792/status/2100474095473455266 (`stardotfun`, `getstonkoptions`, `adamcreates_`).
